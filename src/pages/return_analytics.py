...
def _compute_reconciliation_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _classify_row(row):
        ...
        if wc_st in ["refunded", "cancelled", "failed", "returned", "wc-refunded", "wc-cancelled", "wc-returned"]:
            rec_status = "✅ Verified (WC Refunded/Cancelled)"
        elif "delivered" in p_st:
            rec_status = "🚨 Courier Discrepancy (Pathao Delivered)"
        elif wc_st in ["processing", "shipped", "completed", "confirmed", "wc-shipped", "wc-completed"]:
            rec_status = "⚠️ WC Status Mismatch (Action Needed)"
        else:
            rec_status = "🟡 Pending Verification"

        # 2. Payment Refund Risk
        is_prepaid = any(kw in pmt for kw in ["bkash", "nagad", "rocket", "card", "online", "ssl", "amarpay", "bank"]) or (
            pmt != "" and not any(kw in pmt for kw in ["cod", "cash on delivery", "cash"])
        )
        pmt_flag = "💳 Prepaid (Refund Verification Required)" if is_prepaid else "💵 Cash on Delivery (COD)"
    ...
    classified.columns = ["Reconciliation Status", "Payment Type", "Recommended Action"]
...
def _render_direct_wc_audit_tab():
    ...
    st.caption("Fetch recent orders marked as Refunded, Cancelled, or Failed directly from WooCommerce to verify fulfillment and tracking statuses.")

    c1, c2 = st.columns([2, 1])
    with c1:
        st.info("Query WooCommerce REST API for orders marked with return/refund statuses to verify against Pathao courier data.")
    with c2:
        fetch_direct = st.button("🔄 Audit Live WC Returns", type="primary", use_container_width=True, key="btn_direct_wc_audit")

    if fetch_direct or "direct_wc_returns_df" in st.session_state:
        if fetch_direct:
            with st.status("📡 Fetching WooCommerce refund & cancellation data...", expanded=True) as status_box:
                try:
                    from src.services.woocommerce.client import get_woocommerce_config, HTTPBasicAuth, request_with_backoff, _flatten_order
                    cfg = get_woocommerce_config(required=False)
                    if not cfg:
                        st.error("WooCommerce credentials missing.")
                        return

                    endpoint = f"{cfg.get('store_url', '').rstrip('/')}/wp-json/wc/v3/orders"
                    ...
                    status_box.update(label="📡 Requesting refunded/cancelled orders from WooCommerce...")
                    ...
                    if df_wc_returns.empty:
                        status_box.update(label="ℹ️ No refunded or cancelled orders found in recent records.", state="complete")
                        st.info("No refunded, cancelled, or failed orders found in recent WooCommerce records.")
                        return

                    df_wc_returns = _compute_reconciliation_fields(df_wc_returns)
                    st.session_state["direct_wc_returns_df"] = df_wc_returns
                    status_box.update(label=f"✅ Loaded {len(df_wc_returns)} returned/cancelled items from WooCommerce", state="complete")

                except Exception as e:
                    status_box.update(label="❌ Failed to fetch WooCommerce orders", state="error")
        ...
        if "direct_wc_returns_df" in st.session_state:
            ...
            with col_m2:
                refunded_count = len(df_direct[df_direct["Order Status"].astype(str).str.lower() == "refunded"])
                st.metric("Refunded Status", refunded_count)
            with col_m3:
                cancelled_count = len(df_direct[df_direct["Order Status"].astype(str).str.lower() == "cancelled"])
                st.metric("Cancelled Status", cancelled_count)
            with col_m4:
                tot_val = pd.to_numeric(df_direct.get("Item Cost", 0), errors="coerce").sum()
                st.metric("Refunded Value", f"৳ {tot_val:,.0f}")

            st.markdown("##### 📋 Live WooCommerce Refund Audit Records")
            st.dataframe(
                df_direct[[c for c in ["Order ID", "Order Date", "Order Status", "Payment Method Title", "Full Name (Billing)", "Phone (Billing)", "Item Name", "Item Cost", "Quantity", "Pathao Consignment ID", "Reconciliation Status", "Recommended Action"] if c in df_direct.columns]],
                use_container_width=True,
                hide_index=True,
                column_config={
                    "Item Cost": st.column_config.NumberColumn("Item Cost", format="৳ %d"),
...
def render_return_analytics_tab():
    render_premium_header("Return Analytics", "Track, analyze, and reconcile order returns with WooCommerce & Pathao data", "📉")
    ...
    with st.status("📥 Fetching return data from Google Sheets...", expanded=True) as fetch_status:
        try:
            ...
            with tab_enrich:
                st.markdown("#### 🔄 Real-Time Return Tracking & WooCommerce Cross-Verification")
                ...
                with col1:
                    st.info("Fetch live order details from WooCommerce and tracking statuses from Pathao to audit and verify returns.")
                with col2:
                    if st.button("⚡ Fetch & Enrich Data", use_container_width=True, type="primary"):
                        ...
                        with st.status("🔗 Enriching return data with WooCommerce & Pathao...", expanded=True) as enrich_status:
                            ...
                            try:
                                ...
                                if enable_pathao and "Courier ID" in df_to_match.columns:
                                    ...
                                    enrich_status.update(label=f"🔄 Fetching Pathao statuses (disk-cached) for {len(courier_ids)} orders...")
                                    from src.services.pathao.status import batch_get_pathao_order_statuses
                                    pathao_statuses = batch_get_pathao_order_statuses(courier_ids)
                                ...
                                if not wc_df.empty and "Order Number" in wc_df.columns:
                                    ...
                                    if merged_df.empty:
                                        ...
                                        merged_df = _compute_reconciliation_fields(merged_df)
                ...
                if "enriched_returns" in st.session_state:
                    ...
                    if "Reconciliation Status" in enriched_df.columns:
                        wc_mismatches = len(enriched_df[enriched_df["Reconciliation Status"].astype(str).str.contains("WC Status Mismatch", case=False, na=False)])

                    prepaid_refunds = 0
                    if "Payment Type" in enriched_df.columns:
                        prepaid_refunds = len(enriched_df[enriched_df["Payment Type"].astype(str).str.contains("Prepaid", case=False, na=False)])
            ...
            with tab_recon:
                ...
                st.caption("Audit returned orders against live WooCommerce order statuses and Pathao delivery logs to catch order status mismatches and prepaid refund risks.")

                if "enriched_returns" in st.session_state:
                    rec_df = st.session_state["enriched_returns"].copy()
                    full_recon_df = st.session_state["full_enriched_returns"].copy()

                    filter_opts = ["All Returns", "⚠️ WC Status Mismatches Only", "💳 Prepaid Refund Risks", "🚨 Courier Discrepancies", "✅ Verified Returns"]
                    if hasattr(st, "pills"):
                        sel_filter = st.pills("Filter Audit Matrix", filter_opts, default="All Returns", key="pills_recon_filter")
                    else:
                        sel_filter = st.radio("Filter Audit Matrix", filter_opts, index=0, horizontal=True, key="radio_recon_filter")

                    display_recon = rec_df.copy()
                    if sel_filter == "⚠️ WC Status Mismatches Only":
                        display_recon = display_recon[display_recon["Reconciliation Status"].astype(str).str.contains("WC Status Mismatch", case=False, na=False)]
                    elif sel_filter == "💳 Prepaid Refund Risks":
                        display_recon = display_recon[display_recon["Payment Type"].astype(str).str.contains("Prepaid", case=False, na=False)]
                    elif sel_filter == "🚨 Courier Discrepancies":
                        display_recon = display_recon[display_recon["Reconciliation Status"].astype(str).str.contains("Courier Discrepancy", case=False, na=False)]
                    elif sel_filter == "✅ Verified Returns":
                        display_recon = display_recon[display_recon["Reconciliation Status"].astype(str).str.contains("Verified", case=False, na=False)]

                    search_q = st.text_input("🔍 Search by Order ID, Phone, or Product Name", key="search_recon_matrix").strip()
                    if search_q:
                        display_recon = display_recon[
                            display_recon["Order Number"].astype(str).str.contains(search_q, case=False, na=False)
                            | display_recon.get("Phone (Billing)", pd.Series(dtype=str)).astype(str).str.contains(search_q, case=False, na=False)
                            | display_recon.get("Item Name", pd.Series(dtype=str)).astype(str).str.contains(search_q, case=False, na=False)
                        ]

                    st.markdown(f"**Showing {len(display_recon)} audit records ({sel_filter}):**")
                    st.dataframe(
                        display_recon[[c for c in ["Order Number", "Order Status", "Live Pathao Status", "Reconciliation Status", "Payment Method Title", "Payment Type", "Recommended Action", "Full Name (Billing)", "Phone (Billing)", "Item Name", "Item Cost"] if c in display_recon.columns]],
                        use_container_width=True,
                        hide_index=True,
                        column_config={
                            "Order Status": st.column_config.TextColumn("WooCommerce Status"),
                            "Live Pathao Status": st.column_config.TextColumn("Pathao Status"),
                            "Reconciliation Status": st.column_config.TextColumn("Audit Status"),
                            "Payment Type": st.column_config.TextColumn("Payment Category"),
                            "Recommended Action": st.column_config.TextColumn("Recommended Action"),
                            "Item Cost": st.column_config.NumberColumn("Item Value", format="৳ %d"),
                        },
                    )
                    ...
                    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                        full_recon_df.to_excel(writer, index=False, sheet_name="Matched Returns Overview")

                        mismatches_df = full_recon_df[full_recon_df["Reconciliation Status"].astype(str).str.contains("WC Status Mismatch", case=False, na=False)]
                        if not mismatches_df.empty:
                            mismatches_df.to_excel(writer, index=False, sheet_name="WC Mismatch Action List")

                        prepaid_df = full_recon_df[full_recon_df["Payment Type"].astype(str).str.contains("Prepaid", case=False, na=False)]
                        if not prepaid_df.empty:
                            prepaid_df.to_excel(writer, index=False, sheet_name="Prepaid Refund Audit List")
                    ...
                    st.info("ℹ️ Click **⚡ Fetch & Enrich Data** in the *Enriched Matches* tab to generate the WooCommerce Reconciliation Matrix.")
