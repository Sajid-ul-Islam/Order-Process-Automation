...
def classify_order_source(
    ...
    """
    Classify an order as 'Online' or 'Outlet' based on available data.
    """
    if source_col and source_col in row.index:
        val = str(row[source_col]).strip().lower()
        if val in ["online", "web", "website", "ecom", "e-commerce", "digital"]:
            return "Online"
        if val in ["outlet", "store", "pos", "physical", "retail", "offline"]:
            return "Outlet"
        if "online" in val or "web" in val:
            return "Online"
        if "outlet" in val or "store" in val:
            return "Outlet"

    # Fallback: check Dispatch Suggestion column
    dispatch = str(row.get("Dispatch Suggestion", "")).strip().lower()
    if dispatch and dispatch not in ["nan", "", "ecom-mirpur"]:
        return "Outlet"
