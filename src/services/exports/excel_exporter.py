...
def _detect_column_types(df: pd.DataFrame):
    ...
    currency_cols = [
        ...
        if any(
            kw in str(c).lower()
            for kw in ["amount", "revenue", "cost", "price", "value"]
        )
    ]
    percent_cols = [
        c
        for c in df.columns
        if any(kw in str(c).lower() for kw in ["rate", "percentage", "yield"])
    ]
    stock_cols = [
        c
        for c in df.columns
        if str(c) in ["Ecom-Mirpur", "Wari", "Cumilla", "Sylhet", "Mirpur", "Ecom"]
    ]
...
def _write_row(
    ...
    for c_idx, col_name in enumerate(df.columns):
        ...
        fmt = formatter.get_fmt(
            bg_col, is_top, is_bottom, is_left, is_right, is_curr, is_perc, is_low_stock
        )
...
def export_to_styled_excel(
    df_dict: dict[str, pd.DataFrame], group_by_col: str | None = None
) -> bytes:
    """
