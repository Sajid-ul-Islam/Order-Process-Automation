...
def processed_orders():
    ...
    for order_id, phone, location, quantity in (
        ...
        rows.append({
            ...
            "Phone (Billing)": phone, "First Name (Shipping)": "Test",
            "Last Name (Shipping)": "Buyer", "Address 1&2 (Shipping)": "House 1, Dhanmondi, Dhaka",
            "City (Shipping)": "Dhanmondi", "State Code (Shipping)": "Dhaka",
            "Item Name": "Formal Shirt", "Quantity": quantity, "Item Cost": 100,
            "Order Total Amount": 360 if order_id == "12345" else 160,
            "Payment Method Title": "Cash on delivery", "Dispatch Suggestion": location,
        })
...
def prepared(processed_orders):
    entries, errors = dispatch._prepare_dispatch(
        processed_orders, {"Ecom Mirpur": 11, "Cumilla Outlet": 22}, "test-account",
...
def test_real_processor_uses_correct_store_customer_cod_and_split_parcels(prepared):
    ...
    for entry in prepared:
        ...
        assert payload["store_id"] == (22 if entry["outlet"] == "Cumilla Outlet" else 11)
    ...
    assert next(p for p in payloads if p["merchant_order_id"] == "12345")["item_quantity"] == 2
...
def test_missing_store_and_duplicate_rows_block_batch_validation(processed_orders):
    ...
    _, errors = dispatch._prepare_dispatch(duplicated, {"Cumilla Outlet": 22, "Ecom Mirpur": 11}, "test-account")
    assert len(errors) == 1
...
def test_successful_creation_is_skipped_after_restart(tmp_path, prepared):
    client = Mock()
    client.create_order.side_effect = [{"consignment_id": f"TEST-{i}"} for i in range(3)]
...
def test_retry_sends_only_known_rejections(tmp_path, prepared):
    ...
    client.create_order.side_effect = [
        {"consignment_id": "TEST-1"}, PathaoOrderError("Invalid store"),
        {"consignment_id": "TEST-3"}, {"consignment_id": "TEST-2"},
    ]
    ...
    assert [r["Status"] for r in retry] == ["Already created", "Created", "Already created"]
    assert client.create_order.call_count == 4


@pytest.mark.parametrize("error", [PathaoOrderError("Timed out", uncertain=True), RuntimeError("unexpected")])
...
def test_split_retry_syncs_each_merged_wc_order_once(monkeypatch):
    ...
    assert all("TEST-1, TEST-2" in call.kwargs["note"] for call in update.call_args_list)
    for row in report:
...
def test_wc_failure_keeps_successful_consignment(monkeypatch, tmp_path, prepared):
    ...
    monkeypatch.setattr(dispatch, "update_order_status", Mock(return_value=(False, "WooCommerce unavailable")))
    with DispatchLedger(tmp_path / "dispatch.sqlite3") as ledger:
...
def test_dispatch_screen_store_selection_creation_and_rerun(monkeypatch, tmp_path, processed_orders):
    ...
    client = Mock(base_url="https://pathao.invalid", client_id="test-client", username="test-user")
    client.get_stores.return_value = ([{"store_id": 11, "store_name": "Test Pickup", "is_active": 1}], None)
    client.create_order.return_value = {"consignment_id": "TEST-UI"}
    monkeypatch.setattr(dispatch, "_get_pathao_client", lambda: client)
    monkeypatch.setattr(dispatch, "DispatchLedger", lambda: DispatchLedger(tmp_path / "ui.sqlite3"))
    update = Mock()
    monkeypatch.setattr(dispatch, "update_order_status", update)
    app = AppTest.from_string("from src.pages.pathao_orders.dispatch_tab import _render_auto_dispatch_tab\n_render_auto_dispatch_tab()")
