...
def test_payload_preserves_processor_fields_and_split_instructions(parcel):
    result = build_order_payload(pd.Series(parcel), "42", special_instructions="Call first")
    assert result == {
        "store_id": 42,
    ...
    assert not {"recipient_city", "recipient_zone", "recipient_area"}.intersection(result)
...
@pytest.mark.parametrize("phone", ["+8801712345678", "8801712345678", 1712345678, 1712345678.0, "01712-345678", "০১৭১২৩৪৫৬৭৮"])
...
@pytest.mark.parametrize("field,value", [
    ("MerchantOrderId", ""), ("MerchantOrderId", pd.NA),
    ("RecipientName(*)", "Customer"), ("RecipientName(*)", None),
    ("RecipientPhone(*)", "01700000000"), ("RecipientPhone(*)", "0171***5678"),
    ("RecipientPhone(*)", "01112345678"), ("RecipientPhone(*)", "01712345678 01812345678"),
    ("RecipientAddress(*)", "Address Missing"), ("RecipientAddress(*)", ""),
    ("AmountToCollect(*)", None), ("AmountToCollect(*)", float("nan")),
    ("AmountToCollect(*)", float("inf")), ("AmountToCollect(*)", -1),
    ("ItemQuantity", 1.5), ("ItemQuantity", 0), ("ItemQuantity", True),
    ("ItemWeight", 0), ("ItemWeight", "infinity"), ("ItemWeight", pd.NA),
])
def test_invalid_rows_are_blocked_before_dispatch(parcel, field, value):
    parcel[field] = value
    with pytest.raises(ValueError, match=field.replace("(", r"\(").replace(")", r"\)").replace("*", r"\*")):
        build_order_payload(parcel, 42)


@pytest.mark.parametrize("store_id", [None, 0, -1, 2.5, True, "not-a-store", float("inf")])
...
def test_create_order_sends_single_request_and_requires_consignment(client, parcel, monkeypatch):
    transport = Mock(return_value=response(200, {"type": "success", "data": {"consignment_id": "TEST-CID", "merchant_order_id": "SYNTHETIC-101"}}))
    ...
    transport.assert_called_once_with(
        "POST", "https://pathao.invalid/aladdin/api/v1/orders",
        headers={"Authorization": "Bearer synthetic-token", "Content-Type": "application/json", "Accept": "application/json"},
        json=payload, timeout=30, max_attempts=1, allow_redirects=False,
    )


@pytest.mark.parametrize("document", [None, {}, {"data": {}}, {"data": {"consignment_id": " "}}, {"data": {"consignment_id": None}}, {"data": []}])
...
@pytest.mark.parametrize("status,uncertain", [(400, False), (401, False), (403, False), (408, True), (422, False), (429, False), (500, True), (502, True), (503, True)])
def test_http_failures_are_classified_without_leaking_response(client, monkeypatch, status, uncertain):
    error_response = response(status, {"message": "secret-token", "errors": {"recipient_phone": ["PRIVATE CUSTOMER DATA"], "private_customer": ["PRIVATE"]}})
    transport = Mock(side_effect=HTTPError("secret-token PRIVATE", response=error_response))
    monkeypatch.setattr(client_module, "request_with_backoff", transport)
    with pytest.raises(PathaoOrderError) as error:
        client.create_order({"merchant_order_id": "SYNTHETIC-101"})
...
def test_invalid_json_after_post_is_uncertain(client, monkeypatch):
    ...
    monkeypatch.setattr(client_module, "request_with_backoff", Mock(return_value=result))
    with pytest.raises(PathaoOrderError) as error:
        client.create_order({"merchant_order_id": "SYNTHETIC-101"})
    assert error.value.uncertain
...
def test_semantic_rejection_is_not_reported_as_success(client, monkeypatch):
    monkeypatch.setattr(client_module, "request_with_backoff", Mock(return_value=response(200, {"type": "error", "code": 422, "errors": {"store_id": ["not allowed"]}})))
...
def test_failed_auth_with_expired_token_never_posts_order(client, monkeypatch, refresh_token):
    client.expires_at = 0
...
def test_stores_pagination_loads_all_pickup_choices(client, monkeypatch):
    transport = Mock(side_effect=[
        response(200, {"data": {"data": [{"store_id": 10, "store_name": "Outlet A"}], "current_page": 1, "last_page": 2}}),
        response(200, {"data": {"data": [{"store_id": 20, "store_name": "Outlet B"}], "current_page": 2, "last_page": 2}}),
    ])
    monkeypatch.setattr(client_module, "request_with_backoff", transport)
...
def test_stores_next_page_url_cannot_redirect_credentials(client, monkeypatch):
    transport = Mock(side_effect=[
        response(200, {"data": {"data": [{"store_id": 10, "store_name": "Outlet A"}], "current_page": 1, "next_page_url": "https://untrusted.invalid/collect"}}),
        response(200, {"data": {"data": [{"store_id": 20, "store_name": "Outlet B"}], "current_page": 2, "next_page_url": None}}),
    ])
    monkeypatch.setattr(client_module, "request_with_backoff", transport)
    stores, error = client.get_stores()
    assert error is None and len(stores) == 2
    assert all(call.args[1] == "https://pathao.invalid/aladdin/api/v1/stores" for call in transport.call_args_list)


def test_store_loading_failures_are_actionable_and_sanitized(client, monkeypatch):
    transport = Mock(side_effect=HTTPError("secret-token PRIVATE", response=response(401)))
    monkeypatch.setattr(client_module, "request_with_backoff", transport)
    stores, error = client.get_stores()
    assert stores == []
