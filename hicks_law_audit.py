...
class AuditReport:
    ...
    def to_markdown(self) -> str:
        ...
        for vtype, count in sorted(
            self.violations_by_type.items(), key=lambda x: x[1], reverse=True
        ):
...
class HicksLawAuditor:
    ...
    def _check_multiple_primary_actions(
    ...
    def _check_equal_button_hierarchy(
        self, file_path: Path, content: str, lines: List[str]
    ) -> None:
        ...
        for match in re.finditer(button_group_pattern, content, re.IGNORECASE):
            ...
            btn_matches = re.findall(
                r'<button[^>]*class="([^"]*)"[^>]*>', group_content, re.IGNORECASE
            )

            if len(btn_matches) >= 2:
                # Check if they have similar visual weight
                primary_count = sum(
                    1 for cls in btn_matches if "primary" in cls.lower()
                )
    ...
    def _check_no_visual_hierarchy(
        self, file_path: Path, content: str, lines: List[str]
    ) -> None:
        """Rule 2: Primary button must look significantly more important."""
        # Look for primary buttons without distinctive styling
        primary_btn_pattern = (
            r'<button[^>]*class="[^"]*primary[^"]*"[^>]*>(.*?)</button>'
        )

        for match in re.finditer(
            primary_btn_pattern, content, re.IGNORECASE | re.DOTALL
        ):
            line_num = content[: match.start()].count("\n") + 1
            btn_content = match.group()

            # Check if button lacks visual distinction indicators
            has_size_class = re.search(
                r"(large|big|xl|lg)\b", btn_content, re.IGNORECASE
            )
            has_color_class = re.search(
                r"(blue|green|red|brand|accent)", btn_content, re.IGNORECASE
            )
    ...
    def _check_premature_advanced_options(
        self, file_path: Path, content: str, lines: List[str]
    ) -> None:
        ...
        for pattern in self.ADVANCED_OPTION_PATTERNS:
            ...
            for match in matches:
                ...
                is_visible = not re.search(
                    r"(hidden|collapsed|expand|show|toggle)", context, re.IGNORECASE
                )
                is_on_main_screen = not re.search(
                    r"(modal|dialog|dropdown|accordion)", context, re.IGNORECASE
                )
    ...
    def _check_irrelevant_context_options(
        self, file_path: Path, content: str, lines: List[str]
    ) -> None:
        ...
        if current_context:
            ...
            all_other_contexts = {
                k: v for k, v in context_keywords.items() if k != current_context
            }
    ...
    def _check_hidden_primary_action(
        self, file_path: Path, content: str, lines: List[str]
    ) -> None:
        ...
        has_cta_section = bool(
            re.search(r"(cta|call-to-action|action)", content, re.IGNORECASE)
        )
    ...
    def _generate_report(self) -> AuditReport:
        ...
        if critical_count > 0:
            summary = (
                f"Critical UX friction detected. {critical_count} critical and {high_count} high-"
                f"severity violations found. Immediate refactoring recommended to reduce user "
                f"decision fatigue and improve retention."
            )
        elif high_count > 0:
            summary = (
                f"Significant UX friction identified. {high_count} high-severity violations "
                f"may be causing user hesitation. Prioritize fixing these to streamline user flows."
            )
        else:
            summary = (
                "Minor optimization opportunities found. While no critical issues detected, "
        ...
        if critical_count > 0:
            priority_actions.append(
                "Fix all CRITICAL violations immediately - these block user progress"
            )
        if high_count > 0:
            priority_actions.append(
                "Address HIGH severity violations within current sprint"
            )
        ...
        if violations_by_type.get(ViolationType.MULTIPLE_PRIMARY_ACTIONS, 0) > 0:
            priority_actions.append(
                "Consolidate primary buttons: one screen = one primary action"
            )
        if violations_by_type.get(ViolationType.CLUTTERED_MENU, 0) > 0:
            priority_actions.append(
                "Simplify navigation: group menu items, aim for 5±2 options"
            )
...
class HicksLawRefactorer:
    ...
    def _demote_extra_primary_buttons(
        self, content: str, violation: UXViolation
    ) -> str:
    ...
    def _establish_button_hierarchy(self, content: str, violation: UXViolation) -> str:
    ...
    def _hide_advanced_options(self, content: str, violation: UXViolation) -> str:
        ...
        for pattern in advanced_patterns:
            if pattern in content.lower():
                ...
                replacement = (
                    r"<details><summary>Advanced Options</summary>\1</details>"
                )
                content = re.sub(
                    pattern_re,
                    replacement,
    ...
    def get_refactoring_summary(self) -> str:
        ...
        return f"Refactored {len(self.changes_made)} file(s):\n" + "\n".join(
            f"- {f}" for f in self.changes_made
        )
