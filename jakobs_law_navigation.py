...
class JakobsLawNavigation:
    ...
    def _render_bottom_nav(
        ...
        # 1. Home (far left)
        nav_items.append({"name": "Home", "icon": "🏠", "callback": home_callback})

        # 2. Search/Explore (second from left)
        if search_callback:
            nav_items.append(
                {"name": "Search", "icon": "🔍", "callback": search_callback}
            )
        ...
        if notifications_callback:
            nav_items.append(
                {"name": "Inbox", "icon": "💬", "callback": notifications_callback}
            )

        # 5. Profile (far right)
        nav_items.append(
            {"name": "Profile", "icon": "👤", "callback": profile_callback}
        )
        ...
        for i, item in enumerate(nav_items):
            with cols[i]:
                ...
                if item.get("is_primary"):
                    ...
                    if st.button(
                        label, key=f"nav_{item['name']}", use_container_width=True
                    ):
                        st.session_state.current_page = item["name"]
                        st.rerun()
...
def create_page():
    ...
    st.text_area("What's on your mind?", height=100)
    if st.button("Post"):
